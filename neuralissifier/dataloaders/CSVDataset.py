...
class CSVDataset(Dataset):
    def __init__(self,
        ...
        self.kwargs = dict(N_components=N_components,
                           ...
                           study=study,
                           icn_indices=icn_indices)
        self.icn_indices = icn_indices
        ...
        if study is not None:
            full_data = full_data[full_data['dataset'] == study]
        ...
        if include_sessions.lower() != 'all':
            if type(include_sessions) is str:
                if ',' in include_sessions:
                    ...
                    include_sessions = [include_sessions]
            included_sessions = [
                s for s in included_sessions if s in include_sessions]
        included_files = file_df[file_df[subject_key].isin(included_subjects)]
        self.included_files = included_files[included_files[session_key].isin(
            included_sessions)]
        included_df = full_data[full_data[subject_key].isin(included_subjects)]
        self.included_df = included_df[included_df[session_key].isin(
            included_sessions)]
    ...
    def __getitem__(self, k):
        ...
        if 'spectrogram' in filepath:
            ...
            if self.sequential:
                label = self.labels[k]
                return data.permute([0, 2, 1]).float(), torch.full((self.seqlen,), label).to(data.device), k
            return data.permute([0, 2, 1]).float(), self.labels[k], k
            ...
            try:
                ...
                pass
            if self.icn_indices is not None:                
                #dfnc = dfnc[:, self.icn_indices]
                unflat = torch.zeros((dfnc.shape[0],self.N_components,self.N_components))
                unflat[:, *np.triu_indices(self.N_components, k=1)] = dfnc
                unflat[:, *np.tril_indices(self.N_components, k=-1)] = dfnc
                reduced = unflat[:, self.icn_indices, :]
                reduced = reduced[:, :, self.icn_indices]
                #self.N_components = len(self.icn_indices)
                #self.kwargs['N_components'] = self.N_components
                dfnc = reduced[:, *np.triu_indices(len(self.icn_indices), k=1)]
            ...
            return torch.from_numpy(dfnc), self.labels[k], k


if __name__ == "__main__":
    from torch.utils.data import DataLoader
    test_dataset = CSVDataset(N_subs=99999999999, sequential=False, icn_indices=[0,1,2,3,4])
    test_dataloader = DataLoader(test_dataset, batch_size=4, shuffle=True)
    for batch_i, (fnc, age, k) in enumerate(test_dataloader):
